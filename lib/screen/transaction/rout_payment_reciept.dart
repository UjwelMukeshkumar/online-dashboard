...
import 'package:glowrpt/model/recipt/AccountDataM.dart';
...
class RoutPaymentRecieptScreen extends StatefulWidget {
  bool isRecipt;
  RouteDetailsBean? rootDetails;
  String? empId;
  int? rootId;

  RoutPaymentRecieptScreen({
  ...
  factory RoutPaymentRecieptScreen.create(bool isRecipt) {
    return RoutPaymentRecieptScreen(isRecipt: isRecipt);
  }

  @override
  _RoutPaymentRecieptScreenState createState() => _RoutPaymentRecieptScreenState();
}

enum PayOn { onAccount, chooseBills }

class _RoutPaymentRecieptScreenState extends State<RoutPaymentRecieptScreen> {
  ...
  Future<void> saveData() async {
    ...
    var headder = ReciptHeadderM(
      ...
      PartyType: party1?.Type == null ? "C" : party1!.Type.toString(),
      api_key: compRepo!.getSelectedApiKey(),
      CashAmount: 0,
      CreditAmount: 0,
      CardName: "",
    ...
    if (mode == "Cash") {
      ...
      headder.CashAccount = double.parse(account?.code);
    } else if (mode == "Card") {
      headder.CardName = "";
      headder.CardNum = 0;
    ...
    if (response != null) {
      ...
      // Toast.show("Saved", context, gravity: Toast.CENTER);
      await showDialog(
        context: context,
